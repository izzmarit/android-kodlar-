...
public class MainActivity extends AppCompatActivity implements NetworkDiscoveryManager.DiscoveryCallback {
            ...
            try {
                ...
                runOnUiThread(() -> {
                    if (resolvedIP != null && !resolvedIP.isEmpty()) {
                        // IP adresini kaydet ve bağlantıyı test et
                        ...
                        Log.w(TAG, "mDNS çözümlenemedi, alternatif yöntemlere geçiliyor");
                ...
                runOnUiThread(() -> {
                    updateConnectionStatus(false, "mDNS çözümlenemedi");
                    // mDNS başarısız, diğer yöntemleri dene
                    startNetworkDiscovery();
                });
            } catch (Exception e) {
                Log.e(TAG, "mDNS bağlantı hatası: " + e.getMessage());
                runOnUiThread(() -> {
                    updateConnectionStatus(false, "mDNS hatası");
                    startNetworkDiscovery();
                });
            }
        }).start();
    ...
    private void startDeviceDiscovery() {
        ...
        // İlk olarak mevcut IP ile bağlantı test et
        ...
        // Station modundaysa önce mDNS dene
        if (connectionMode == Constants.MODE_STATION) {
            tryMDNSConnection();
            return;
        }
        ...
        if (connectionMode == Constants.MODE_AP) {
            if (Constants.DEFAULT_AP_IP.equals(currentIP)) {
