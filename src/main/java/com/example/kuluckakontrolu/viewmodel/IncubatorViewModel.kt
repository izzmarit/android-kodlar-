...
class IncubatorViewModel(private val repository: IncubatorRepository) : ViewModel() {
    ...
    fun connectToIncubator(retryCount: Int = 3) {
        viewModelScope.launch {
            ...
            try {
                // Log ekle
                Log.d("ViewModel", "ESP32'ye bağlanmaya çalışılıyor...")
                ...
                if (connected) {
                    ...
                    repository.monitorData()
                        ...
                        .catch { e ->
                            Log.e("ViewModel", "Veri akışı hatası: ${e.message}")
                    ...
                    repository.monitorErrors()
                        .onEach { error ->
                            Log.e("ViewModel", "ESP32 hatası: $error")
                            if (error is IncubatorError.CONNECTION_ERROR) {
                                _connectionStatus.value = ConnectionStatus.FAILED
                            }
                            _dataState.value = LoadingState.Error(error)
                        }
                        .launchIn(viewModelScope)
                } else {
                    _connectionStatus.value = ConnectionStatus.FAILED
                ...
                Log.e("ViewModel", "Bağlantı hatası: ${e.message}")
                _connectionStatus.value = ConnectionStatus.FAILED
                _dataState.value = LoadingState.Error(IncubatorError.GENERIC_ERROR(e.message ?: "Bilinmeyen hata"))
            }
        }
