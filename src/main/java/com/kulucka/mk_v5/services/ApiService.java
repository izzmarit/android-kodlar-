...
public class ApiService {
    ...
    // ESP32'nin createAppData fonksiyonundan tüm verileri al
    public void getAppData(DataCallback callback) {
        String url = baseUrl + "/api/status";

        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.GET, url, null,
                response -> {
                    Log.d(TAG, "ESP32 app data received");
                    if (callback != null) callback.onSuccess(response);
                },
                error -> {
                    Log.e(TAG, "Failed to get ESP32 app data: " + error.getMessage());
                    if (callback != null) callback.onError(getVolleyErrorMessage(error));
                }
        );
    ...
    // WiFi durumunu al
    public void getWiFiStatus(DataCallback callback) {
        String url = baseUrl + "/api/status";

        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.GET, url, null,
                response -> {
                    Log.d(TAG, "ESP32 WiFi status received");
                    if (callback != null) callback.onSuccess(response);
                },
                error -> {
                    Log.e(TAG, "Failed to get ESP32 WiFi status: " + error.getMessage());
                    if (callback != null) callback.onError(getVolleyErrorMessage(error));
                }
        );
    ...
    // WiFi ağlarını tara
    public void scanWiFiNetworks(DataCallback callback) {
        String url = baseUrl + "/api/wifi/networks";

        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.GET, url, null,
                response -> {
                    Log.d(TAG, "ESP32 WiFi networks scanned");
                    if (callback != null) callback.onSuccess(response);
                },
                error -> {
                    Log.e(TAG, "Failed to scan ESP32 WiFi networks: " + error.getMessage());
                    if (callback != null) callback.onError(getVolleyErrorMessage(error));
                }
        );
    ...
    // WiFi'ye bağlan (JSONObject parametreli)
    public void connectToWiFi(JSONObject wifiParams, ParameterCallback callback) {
        String url = baseUrl + "/api/wifi/connect";

        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.POST, url, wifiParams,
                response -> {
                    Log.d(TAG, "ESP32 WiFi connection initiated");
                    if (callback != null) callback.onSuccess();
                },
    ...
    // AP ayarlarını güncelle
    public void updateAPSettings(JSONObject apParams, ParameterCallback callback) {
        String url = baseUrl + "/api/wifi/ap";

        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.POST, url, apParams,
                response -> {
                    Log.d(TAG, "ESP32 AP settings updated");
                    if (callback != null) callback.onSuccess();
                },
                error -> {
                    Log.e(TAG, "Failed to update ESP32 AP settings: " + error.getMessage());
                    if (callback != null) callback.onError(getVolleyErrorMessage(error));
                }
        );
    ...
    // WiFi ağını unut
    public void forgetWiFiNetwork(ParameterCallback callback) {
        String url = baseUrl + "/api/wifi/forget";

        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.POST, url, null,
                response -> {
                    Log.d(TAG, "ESP32 WiFi network forgotten");
                    if (callback != null) callback.onSuccess();
                },
                error -> {
                    Log.e(TAG, "Failed to forget ESP32 WiFi network: " + error.getMessage());
                    if (callback != null) callback.onError(getVolleyErrorMessage(error));
                }
        );
    ...
    public void updateSettings(JSONObject settings, ParameterCallback callback) {
        String url = baseUrl + "/api/settings";

        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.POST, url, settings,
                response -> {
                    Log.d(TAG, "ESP32 settings updated successfully");
                    if (callback != null) callback.onSuccess();
                },
                error -> {
                    Log.e(TAG, "Failed to update ESP32 settings: " + error.getMessage());
                    if (callback != null) callback.onError(getVolleyErrorMessage(error));
                }
        );
    ...
    // Kuluçka başlat (JSONObject parametreli)
    public void startIncubation(JSONObject incubationParams, ParameterCallback callback) {
        String url = baseUrl + "/api/incubation";

        try {
            incubationParams.put("isIncubationRunning", true);
        } catch (JSONException e) {
            Log.e(TAG, "JSON error: " + e.getMessage());
            if (callback != null) callback.onError("JSON error: " + e.getMessage());
            return;
        ...
        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.POST, url, incubationParams,
                response -> {
                    Log.d(TAG, "ESP32 incubation started");
                    if (callback != null) callback.onSuccess();
                },
                error -> {
                    Log.e(TAG, "Failed to start ESP32 incubation: " + error.getMessage());
                    if (callback != null) callback.onError(getVolleyErrorMessage(error));
                }
        );
    ...
    // Kuluçka durdur (parametresiz)
    public void stopIncubation(ParameterCallback callback) {
        String url = baseUrl + "/api/incubation";

        try {
            JSONObject jsonBody = new JSONObject();
            jsonBody.put("isIncubationRunning", false);

            JsonObjectRequest request = new JsonObjectRequest(
                    Request.Method.POST, url, jsonBody,
                    response -> {
                        Log.d(TAG, "ESP32 incubation stopped");
                        if (callback != null) callback.onSuccess();
                    },
                    error -> {
                        Log.e(TAG, "Failed to stop ESP32 incubation: " + error.getMessage());
                        if (callback != null) callback.onError(getVolleyErrorMessage(error));
                    }
            );
    ...
    // Alarm testleri
    public void testAlarms(ParameterCallback callback) {
        String url = baseUrl + "/api/test/alarm";

        JsonObjectRequest request = new JsonObjectRequest(
                Request.Method.POST, url, null,
                response -> {
                    Log.d(TAG, "ESP32 alarm test successful");
                    if (callback != null) callback.onSuccess();
                },
                error -> {
                    Log.e(TAG, "Failed to test ESP32 alarms: " + error.getMessage());
                    if (callback != null) callback.onError(getVolleyErrorMessage(error));
                }
        );
