...
public class ESP32ConnectionService extends Service {
    ...
    private static final long CHECK_INTERVAL = 30000; // 30 saniye
    private static final String ESP32_AP_SSID = "KULUCKA_MK_v5"; // ESP32'deki AP_SSID ile eşleşmeli
    private static final String ESP32_AP_PASSWORD = "12345678"; // ESP32'deki AP_PASS ile eşleşmeli
    ...
    private void checkAndMaintainConnection() {
        ...
        if (ESP32_AP_SSID.equals(currentSSID)) {
            ...
            if (!savedIP.isEmpty() && !savedIP.equals("192.168.4.1")) {
                ...
                // ESP32'den güncel durumu al
                getCurrentESP32Status();
            } else {
                // ESP32 bulunamadı veya kayıtlı IP yok
    ...
    private void testESP32Connection() {
        ...
        // ESP32'nin /api/status endpoint'ini test et
    ...
    private void handleConnectionSuccess() {
        ...
        if (!wasConnected) {
            ...
            // ApiService'e güncel IP'yi ayarla
            ApiService.getInstance().setBaseUrl("http://" + currentIP);
        }
    }
    ...
    private void getCurrentESP32Status() {
        // ESP32'den güncel WiFi ve sistem durumunu al
        ApiService.getInstance().getStatus(new ApiService.DataCallback() {
            @Override
            public void onSuccess(JSONObject data) {
                updateESP32StatusFromData(data);
            }

            @Override
            public void onError(String message) {
                Log.w(TAG, "ESP32 status check failed: " + message);
                handleConnectionFailure();
            }
        });
    }

    private void updateESP32StatusFromData(JSONObject data) {
        try {
            // ESP32'nin WiFi durumu bilgilerini güncelle
            ...
            boolean isConnected = data.optBoolean("systemOk", true);

            // Mevcut durum bilgilerini güncelle
            if ("Station".equals(wifiMode) && !ipAddress.equals("192.168.4.1")) {
                currentMode = "Station";
                currentIP = ipAddress;
            } else {
                currentMode = "AP";
            ...
            if (isConnected) {
                ...
                // IP adresi değiştiyse güncelle
                if (!currentIP.equals("192.168.4.1")) {
                    updateMainActivityIP(currentIP);
                }
            } else {
                handleConnectionFailure();
            }
            ...
            Log.e(TAG, "Error processing ESP32 status data: " + e.getMessage());
            handleConnectionFailure();
        }
    }

    private void updateMainActivityIP(String newIP) {
        // MainActivity'ye IP adresini bildir (intent ile)
        try {
            Intent intent = new Intent("com.kulucka.mk_v5.IP_UPDATE");
            intent.putExtra("new_ip", newIP);
            sendBroadcast(intent);

            Log.d(TAG, "New IP address broadcasted: " + newIP);
        } catch (Exception e) {
            Log.e(TAG, "Error broadcasting IP update: " + e.getMessage());
        }
    }
}