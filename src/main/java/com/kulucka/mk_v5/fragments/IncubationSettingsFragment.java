...
public class IncubationSettingsFragment extends Fragment {
    ...
    private void saveSettings() {
        try {
            ...
            try {
                ...
                jsonData.put("incubationType", incubationType);

                // Manuel mod ise parametreleri ekle
                if (incubationType == Constants.INCUBATION_MANUAL) {
                    ...
                    jsonData.put("manualDevTemp", devTemp);
                    jsonData.put("manualHatchTemp", hatchTemp);
                    jsonData.put("manualDevHumid", devHumid);
                    jsonData.put("manualHatchHumid", hatchHumid);
                    jsonData.put("manualDevDays", devDays);
                    jsonData.put("manualHatchDays", hatchDays);
                }

                ApiService.getInstance().setParameter("incubation", jsonData.toString(), new ApiService.ParameterCallback() {
                    @Override
                    public void onSuccess() {
                        showToast("Kuluçka ayarları başarıyla kaydedildi");
                    }
    ...
    private void toggleIncubation() {
        try {
            // JSON verisi oluştur
            org.json.JSONObject jsonData = new org.json.JSONObject();
            jsonData.put("isIncubationRunning", !isIncubationRunning);
