...
public class PidSettingsFragment extends Fragment {
    ...
    // ESP32 Varsayılan Sıcaklık PID Değerleri
                ...
                if (getActivity() != null) {
                    getActivity().runOnUiThread(() -> {
                        try {
                            ...
                            double kp = data.optDouble("kp", DEFAULT_TEMP_KP);
                            double ki = data.optDouble("ki", DEFAULT_TEMP_KI);
                            double kd = data.optDouble("kd", DEFAULT_TEMP_KD);
    ...
    private void updateModeUI(int checkedId) {
        ...
        etTempKp.setEnabled(isManualMode);
        etTempKi.setEnabled(isManualMode);
        etTempKd.setEnabled(isManualMode);
        btnSavePidSettings.setEnabled(isManualMode);
        btnLoadDefaults.setEnabled(isManualMode);

        // Otomatik ayarlama butonu
        btnStartAutoTune.setEnabled(isAutoMode);

        // Görsel feedback
        float manualAlpha = isManualMode ? 1.0f : 0.5f;
        float autoAlpha = isAutoMode ? 1.0f : 0.5f;
    ...
    private void savePidSettings() {
        try {
            ...
            // ESP32'ye gönderilecek JSON formatı
            JSONObject jsonData = new JSONObject();
            jsonData.put("pidEnabled", swPidEnable.isChecked());
            jsonData.put("pidMode", "manual");
            jsonData.put("kp", tempKp);
            jsonData.put("ki", tempKi);
            jsonData.put("kd", tempKd);

            showToast("PID ayarları ESP32'ye gönderiliyor...");

            ApiService.getInstance().setParameter("pid/set", jsonData.toString(), new ApiService.ParameterCallback() {
                @Override
                ...
                public void onSuccess() {
                    ...
                    if (getActivity() != null) {
                        getActivity().runOnUiThread(() -> {
                            ...
                            // 2 dakika sonra butonu tekrar aktif et ve durumu kontrol et
