...
public interface ApiService {
    ...
    // ApiService.java içine eklenecek OTA endpoint'leri

    // OTA bilgileri
    @GET("/api/ota/info")
    Call<OTAInfoResponse> getOTAInfo();

    // OTA güncelleme
    @Multipart
    @POST("/api/ota/update")
    Call<ResponseBody> uploadFirmware(@Part MultipartBody.Part file,
                                      @Header("X-MD5") String md5);

    // OTA ilerleme durumu
    @GET("/api/ota/progress")
    Call<OTAProgressResponse> getOTAProgress();

    // OTA Response sınıfları
    public static class OTAInfoResponse {
        ...
        public void setChipModel(String chipModel) { this.chipModel = chipModel; }

        public int getCpuFreq() { return cpuFreq; }
        public void setCpuFreq(int cpuFreq) { this.cpuFreq = cpuFreq; }
    }

    public static class OTAProgressResponse {
        private int state;
        ...
        public long getWrittenSize() { return writtenSize; }
        public void setWrittenSize(long writtenSize) { this.writtenSize = writtenSize; }
