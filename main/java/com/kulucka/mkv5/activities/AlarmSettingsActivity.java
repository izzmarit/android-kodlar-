...
public class AlarmSettingsActivity extends AppCompatActivity {
    ...
    private void updateUI() {
        ...
        // Alarms objesini kontrol et
        if (currentStatus.getAlarms() != null) {
            DeviceStatus.AlarmsInfo alarms = currentStatus.getAlarms();

            switchAlarmEnabled.setChecked(alarms.isEnabled());
            ...
            // ESP32'den gelen gerçek limit değerlerini kullan
            float tempHighOffset = alarms.getTempHigh() - currentStatus.getTargetTemp();
            float tempLowOffset = currentStatus.getTargetTemp() - alarms.getTempLow();
            float humidHighOffset = alarms.getHumidHigh() - currentStatus.getTargetHumid();
            float humidLowOffset = currentStatus.getTargetHumid() - alarms.getHumidLow();

            // Negatif değerleri pozitife çevir ve göster
            tempHighOffset = Math.abs(tempHighOffset);
            tempLowOffset = Math.abs(tempLowOffset);
            humidHighOffset = Math.abs(humidHighOffset);
            humidLowOffset = Math.abs(humidLowOffset);
            ...
            prefsManager.saveTempHighOffset(tempHighOffset);
            ...
            // Eski yöntemle offset hesapla
            float tempHighOffset = Math.abs(currentStatus.getTempHighAlarm() - currentStatus.getTargetTemp());
            float tempLowOffset = Math.abs(currentStatus.getTargetTemp() - currentStatus.getTempLowAlarm());
            float humidHighOffset = Math.abs(currentStatus.getHumidHighAlarm() - currentStatus.getTargetHumid());
            float humidLowOffset = Math.abs(currentStatus.getTargetHumid() - currentStatus.getHumidLowAlarm());
        ...
        enableDisableInputs(switchAlarmEnabled.isChecked());
        calculateAndDisplayLimits();
    }
