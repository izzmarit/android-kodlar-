...
public class CalibrationSettingsActivity extends AppCompatActivity {
    ...
    private void updateUI() {
        ...
        // Sensör detaylarını kontrol et
        if (currentStatus.getSensors() != null) {
            DeviceStatus.SensorsInfo sensors = currentStatus.getSensors();

            // Sensor 1
            if (sensors.getSensor1() != null) {
                DeviceStatus.SensorsInfo.SensorDetail sensor1 = sensors.getSensor1();
                tvSensor1Temp.setText(String.format("%.1f°C", sensor1.getTemperature()));
                tvSensor1Humid.setText(String.format("%d%%", Math.round(sensor1.getHumidity())));
                etSensor1TempCal.setText(String.format("%.1f", sensor1.getTempCalibration()).replace('.', ','));
                etSensor1HumidCal.setText(String.format("%.1f", sensor1.getHumidCalibration()).replace('.', ','));
            } else {
                // Eski yöntem
            ...
            // Sensor 2
            if (sensors.getSensor2() != null) {
                DeviceStatus.SensorsInfo.SensorDetail sensor2 = sensors.getSensor2();
                tvSensor2Temp.setText(String.format("%.1f°C", sensor2.getTemperature()));
                tvSensor2Humid.setText(String.format("%d%%", Math.round(sensor2.getHumidity())));
                etSensor2TempCal.setText(String.format("%.1f", sensor2.getTempCalibration()).replace('.', ','));
                etSensor2HumidCal.setText(String.format("%.1f", sensor2.getHumidCalibration()).replace('.', ','));
            } else {
                // Eski yöntem
