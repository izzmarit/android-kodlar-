...
public class WifiSettingsActivity extends AppCompatActivity {
    ...
    private void connectToWifi(String ssid, String password) {
        // Önce kaydedilmiş şifre var mı kontrol et
        String savedPassword = prefsManager.getWifiPassword(ssid);

        // Final değişken oluştur
        final String finalPassword;
        if (!savedPassword.isEmpty() && password.isEmpty()) {
            // Kaydedilmiş şifre varsa kullan
            finalPassword = savedPassword;
            Log.d(TAG, "Kaydedilmiş şifre kullanılıyor: " + ssid);
        } else {
        ...
        if (!finalPassword.isEmpty()) {
            prefsManager.saveWifiNetwork(ssid, finalPassword);
            Log.d(TAG, "WiFi credential'ları kaydedildi: " + ssid);
        }

        networkManager.connectToWifi(ssid, finalPassword, new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if (response.isSuccessful()) {
                    ...
                    // HTTP hatası durumunda da kaydedilmiş şifreyi sil
                    if (!savedPassword.isEmpty()) {
                        prefsManager.removeWifiNetwork(ssid);
                        Log.d(TAG, "HTTP hatası nedeniyle şifre silindi: " + ssid);
                    }
                }
            }
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                ...
                // Ağ hatası durumunda kaydedilmiş şifreyi kontrol et
                if (savedPassword.isEmpty() && !finalPassword.isEmpty()) {
                    // Sadece yeni girilen şifre başarısızsa kaydetme
                    prefsManager.removeWifiNetwork(ssid);
                    Log.d(TAG, "Network hatası nedeniyle yeni şifre kaydedilmedi: " + ssid);
                }
            }
        });
    }
    ...
    private void performSystemVerification() {
        ...
        final int maxRetries = 5; // Retry sayısı artırıldı
